...
public:
    ...
    ResultVector(const std::vector<double>& x_plot, const std::vector<double>& y_plot, const std::vector<std::complex<double>>& complex, double samples) :x(x_plot), y(y_plot), j(complex), samples(samples) {};
    ...
    ResultVector operator+(const ResultVector& other) const { //jeœli ta sama wielkoœæ wektorów to zsumuj je za pomoc¹ (przeci¹¿onego) "+"
        ...
        if (x != other.x) {
            std::cout << "sizes differ! ";
            sum = { 0 };
            return ResultVector(x, sum, cplx0, samples);
        }
    ...
    ResultVector operator*(double scalar) const {//this -> Result vector, czyli x i y, other -> scalar
        ...
        return ResultVector(x, multiplication, cplx0, samples); // wynik wektor * skalar
    }
...
PYBIND11_MODULE(_core, m) {
    py::class_<ResultVector>(m, "ResultVector")
        .def(py::init<>())
        .def(py::init<const std::vector<double>&, const std::vector<double>&, const std::vector<std::complex<double>>&, double>())
        .def_readwrite("x", &ResultVector::x)
        .def_readwrite("y", &ResultVector::y)
        .def_readwrite("j", &ResultVector::j)
        .def_readwrite("samples", &ResultVector::samples)
    ...
    m.def("sin", [](double f, double start, double end, int samples) {
        ...
        ResultVector sinPlot(fsin, ysin, cplx0, samples);
        return sinPlot;
    ...
    m.def("cos", [](double f, double start, double end, int samples) {
        ...
        ResultVector cosPlot(fcos, ycos, cplx0, samples);
        return cosPlot;
        }, py::arg("frequency"), py::arg("start"), py::arg("end"), py::arg("samples"), R"pbdoc(
        Create cosinus plot
    )pbdoc");
    ...
    m.def("sqrwave", [](double f, double start, double end, int sample) {
        ...
        ResultVector sqwPlot(fsqw, ysqw, cplx0,sample);
        return sqwPlot;
        }, py::arg("frequency"), py::arg("start"), py::arg("end"), py::arg("samples"), R"pbdoc(
    ...
    m.def("sawwave", [](double f, double start, double end, int sample) {
        ...
        ResultVector sawPlot(fsaw, ysaw, cplx0,sample);
        return sawPlot;
        }, py::arg("frequency"), py::arg("start"), py::arg("end"), py::arg("samples"), R"pbdoc(
        Create sawwave plot
    )pbdoc");
    ...
    m.def("fourier", [](ResultVector testPlot) {
        ...
        for (int k = 0; k < samples; ++k) {
            ...
            for (int n = 0; n < samples; ++n) {
                double angle = 2.0 * matplot::pi * (double(k) / double(samples)) * n;
                fourierX += testPlot.y[n] * (std::exp(-i * angle));
            }
            complexVec.push_back(fourierX);
            if (k < halfSampl)y.push_back(std::abs(fourierX));
            else y.push_back(0);
        }
        ResultVector dftPlot(x, y, complexVec, size);//tutaj size to rozmiar pierwotnej funkcji - tak lepiej żeby było oryginalnie a samples pobierać z rozmiaru wektora x
        return dftPlot;
        }, py::arg("plot"), R"pbdoc(
    ...
    m.def("frequenciesDFT", [](ResultVector testPlot, int end, int sampleSize, int range2pi) {
        ...
        ResultVector shortPlot(x, y, testPlot.j,sampleSize);
        return shortPlot;
    ...
    m.def("inv_fourier", [](ResultVector testPlot) {
        int seqNr = testPlot.x.size();
        std::vector<double> x = matplot::linspace(0, testPlot.samples, seqNr);
        std::vector<double> y;
        std::vector<std::complex<double>> complexVec;
        ...
        ResultVector idftPlot(x, y, complexVec, testPlot.samples);
        return idftPlot;
        }, py::arg("DFT"), R"pbdoc(
    ...
    m.def("filter1D", [](ResultVector testPlot) {
        ...
        ResultVector filteredPlot(testPlot.x, out_plot, testPlot.j, testPlot.samples);
        return filteredPlot;
        }, py::arg("plot"), R"pbdoc(
    ...
    m.def("correlation", [](ResultVector testPlot1, ResultVector testPlot2) {
        ...
        if (testPlot1.x != testPlot2.x) {
            std::cout << "sizes differ!";
            out_plot = { 0 };
            return ResultVector(testPlot1.x, out_plot, cplx0, testPlot1.samples);
        }
        ...
        ResultVector correlationPlot(testPlot1.x, out_plot, cplx0, testPlot1.samples);
